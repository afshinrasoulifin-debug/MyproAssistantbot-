...
"""Real behavioral tests for middlewares/poison_pill_middleware.py"""
import pytest
...
def _import():
    try:
        import importlib
        return importlib.import_module("tg_bot.middlewares.poison_pill_middleware")
    except (ImportError, ModuleNotFoundError) as e:
        pytest.skip(f"Cannot import: {e}")


class TestPoisonPillMiddleware:
    """Tests for PoisonPillMiddleware middleware."""

    def test_instantiate(self):
        mod = _import()
        mw = mod.PoisonPillMiddleware(MagicMock())
