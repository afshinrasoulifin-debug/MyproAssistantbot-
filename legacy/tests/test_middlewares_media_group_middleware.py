...
"""Real behavioral tests for middlewares/media_group_middleware.py"""
import pytest
...
def _import():
    try:
        import importlib
        return importlib.import_module("tg_bot.middlewares.media_group_middleware")
    except (ImportError, ModuleNotFoundError) as e:
        pytest.skip(f"Cannot import: {e}")


class TestMediaGroupMiddleware:
    """Tests for MediaGroupMiddleware middleware."""

    def test_instantiate(self):
        mod = _import()
        mw = mod.MediaGroupMiddleware(MagicMock())
