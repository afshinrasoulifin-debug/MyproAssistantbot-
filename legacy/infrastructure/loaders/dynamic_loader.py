...
"""DynamicLoader — Hot-load components at runtime."""
...
class DynamicLoader:
    ...
    def __init__(self, *, search_paths: Optional[List[str]] = None) -> None:
        ...
        logger.info("DynamicLoader initialized (paths=%s)", self._search_paths)
    ...
    async def load(self, name: str, *, reload: bool = False) -> Optional[Any]:
        ...
        for sp in self._search_paths:
            try:
                ...
                logger.info("DynamicLoader loaded: %s", name)
                return self._loaded[name]
            except Exception as e:
                self._errors.append({"name": name, "error": str(e), "at": time.time()})

        logger.warning("DynamicLoader failed to load: %s", name)
        return None
