...
"""InfraRuntimeLoader — Load runtime components dynamically."""
...
class InfraRuntimeLoader:
    ...
    def __init__(self, *, search_paths: Optional[List[str]] = None) -> None:
        ...
        logger.info("InfraRuntimeLoader initialized (paths=%s)", self._search_paths)
    ...
    async def load(self, name: str, *, reload: bool = False) -> Optional[Any]:
        ...
        for sp in self._search_paths:
            try:
                ...
                logger.info("InfraRuntimeLoader loaded: %s", name)
                return self._loaded[name]
            except Exception as e:
                self._errors.append({"name": name, "error": str(e), "at": time.time()})

        logger.warning("InfraRuntimeLoader failed to load: %s", name)
        return None
