...
"""LiveService — Real-time data service."""
...
class LiveService:
    ...
    def __init__(self, *, name: str = "live_service") -> None:
        self.name = name
        self._running = False
        self._handlers: Dict[str, Any] = {}
        self._queue: List[Dict] = []
        self._stats = {"processed": 0, "errors": 0, "total_ms": 0.0}
        logger.info("LiveService '%s' initialized", name)

    async def start(self) -> None:
        self._running = True
        logger.info("LiveService '%s' started", self.name)

    async def stop(self) -> None:
        self._running = False
        logger.info("LiveService '%s' stopped", self.name)
    ...
    async def process(self, action: str, data: Any = None) -> Dict:
        ...
        try:
            ...
            logger.error("LiveService process error: %s", e)
