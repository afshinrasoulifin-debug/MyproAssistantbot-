...
"""ModelWrapper — Wrap individual models with optimization."""
...
class ModelWrapper:
    ...
    def __init__(self, wrapped: Any = None, *, name: str = "model_wrapper") -> None:
        self._wrapped = wrapped
        self.name = name
        self._before_hooks: List = []
        self._after_hooks: List = []
        self._stats = {"calls": 0, "errors": 0, "total_ms": 0.0}
        logger.info("ModelWrapper '%s' initialized", name)

    def before(self, hook: Any) -> "ModelWrapper":
        """Add a before hook."""
        self._before_hooks.append(hook)
        return self

    def after(self, hook: Any) -> "ModelWrapper":
        """Add an after hook."""
    ...
    async def call(self, method: str, *args, **kwargs) -> Dict:
        ...
        for hook in self._before_hooks:
            try:
                ...
                logger.warning("ModelWrapper before hook error: %s", e)
        ...
        for hook in self._after_hooks:
            try:
                ...
                logger.warning("ModelWrapper after hook error: %s", e)
