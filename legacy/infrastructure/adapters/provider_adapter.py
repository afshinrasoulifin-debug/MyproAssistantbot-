...
"""InfraProviderAdapter — Adapt provider interfaces."""
...
class InfraProviderAdapter:
    ...
    def __init__(self, client: Any = None, *, config: Optional[Dict] = None) -> None:
        ...
        logger.info("InfraProviderAdapter initialized (config=%d keys)", len(self._config))

    def add_transform(self, fn: Any) -> "InfraProviderAdapter":
    ...
    async def adapt_request(self, request: dict) -> dict:
        ...
        for fn in self._transforms:
            try:
                ...
                logger.warning("InfraProviderAdapter transform error: %s", e)
    ...
    async def adapt_response(self, response: dict) -> dict:
        ...
        result.setdefault("_adapter", "InfraProviderAdapter")
