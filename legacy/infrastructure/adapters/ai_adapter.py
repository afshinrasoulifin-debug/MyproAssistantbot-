...
"""InfraAIAdapter — Adapt AI client interfaces."""
...
class InfraAIAdapter:
    ...
    def __init__(self, client: Any = None, *, config: Optional[Dict] = None) -> None:
        ...
        logger.info("InfraAIAdapter initialized (config=%d keys)", len(self._config))

    def add_transform(self, fn: Any) -> "InfraAIAdapter":
    ...
    async def adapt_request(self, request: dict) -> dict:
        ...
        for fn in self._transforms:
            try:
                ...
                logger.warning("InfraAIAdapter transform error: %s", e)
    ...
    async def adapt_response(self, response: dict) -> dict:
        ...
        result.setdefault("_adapter", "InfraAIAdapter")
