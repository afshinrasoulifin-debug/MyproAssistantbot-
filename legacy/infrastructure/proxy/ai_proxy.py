...
"""AIProxy — Transparent proxy for AI API calls."""
...
class AIProxy:
    ...
    def __init__(self, *, name: str = "ai_proxy") -> None:
        self.name = name
        self._targets: Dict[str, str] = {}
        self._filters: List = []
        self._stats = {"proxied": 0, "filtered": 0, "errors": 0, "bytes": 0}
        logger.info("AIProxy '%s' initialized", name)
    ...
    async def proxy(self, target: str, request: Dict) -> Dict:
        ...
        for filt in self._filters:
            try:
                ...
                logger.warning("AIProxy filter error: %s", e)
