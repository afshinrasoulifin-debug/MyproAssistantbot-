...
"""ReverseProxy — Reverse proxy for incoming API requests."""
...
class ReverseProxy:
    ...
    def __init__(self, *, name: str = "reverse_proxy") -> None:
        self.name = name
        self._targets: Dict[str, str] = {}
        self._filters: List = []
        self._stats = {"proxied": 0, "filtered": 0, "errors": 0, "bytes": 0}
        logger.info("ReverseProxy '%s' initialized", name)
    ...
    async def proxy(self, target: str, request: Dict) -> Dict:
        ...
        for filt in self._filters:
            try:
                ...
                logger.warning("ReverseProxy filter error: %s", e)
