...
"""RequestProxy — HTTP request proxy with transformation."""
...
class RequestProxy:
    ...
    def __init__(self, *, name: str = "request_proxy") -> None:
        self.name = name
        self._targets: Dict[str, str] = {}
        self._filters: List = []
        self._stats = {"proxied": 0, "filtered": 0, "errors": 0, "bytes": 0}
        logger.info("RequestProxy '%s' initialized", name)
    ...
    async def proxy(self, target: str, request: Dict) -> Dict:
        ...
        for filt in self._filters:
            try:
                ...
                logger.warning("RequestProxy filter error: %s", e)
