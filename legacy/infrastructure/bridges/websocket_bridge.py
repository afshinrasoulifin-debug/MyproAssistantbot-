...
"""InfraWebSocketBridge — WebSocket bridge for real-time."""
...
class InfraWebSocketBridge:
    ...
    def __init__(self) -> None:
        ...
        logger.info("InfraWebSocketBridge initialized")
    ...
    def connect(self, name: str, target: Any, *, config: Optional[Dict] = None) -> None:
        ...
        logger.info("InfraWebSocketBridge connected: %s", name)
    ...
    async def send(self, target: str, data: Any) -> Dict:
        ...
        try:
            ...
            logger.debug("InfraWebSocketBridge sent %d bytes to %s", size, target)
            return {"ok": True, "target": target, "size": size}
        except Exception as e:
            self._stats["errors"] += 1
            logger.error("InfraWebSocketBridge send error: %s", e)
