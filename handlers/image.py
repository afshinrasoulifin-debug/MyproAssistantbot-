...
# ═══════════════════════════════════════
# /hd — HD Image Generation (v30.1)
# ═══════════════════════════════════════

@router.message(Command("hd"))
async def cmd_hd_image(message: Message) -> None:
    """Generate a high-definition image with model selection."""
    from arki_project.utils.image_gen import generate_image_hd, IMAGE_MODELS

    prompt = extract_args(message.text or "", "/hd")
    if not prompt:
        models_list = "\n".join(f"  `{k}` — {k}" for k in IMAGE_MODELS)
        await safe_reply(message, "🖼 *تولید عکس HD:*\n\n"
            "Usage: `/hd [توضیح] | [مدل]`\n\n"
            f"*مدل‌های موجود:*\n{models_list}\n\n"
            "*مثال:*\n"
            "`/hd a luxury candle on marble table | flux-realism`\n"
            "`/hd لوگو مینیمال | flux`\n"
            "`/hd anime girl with candles | flux-anime`\n"
            "`/hd 3D render of a candle | flux-3d`\n\n"
            "_مدل پیش‌فرض: flux-realism (واقع‌گرایانه)_")
        return

    parts = [p.strip() for p in prompt.split("|")]
    actual_prompt = parts[0]
    model = parts[1].lower().strip() if len(parts) > 1 else "flux-realism"
    ...
    status = await message.answer(f"🖼 ساخت عکس HD ({model})...")

    try:
        image_bytes = await generate_image_hd(
            actual_prompt, model=model, width=1024, height=1024,
        )
        await _track_tokens(message.from_user.id if message.from_user else 0, actual_prompt, extra_tokens=1500)
        photo = BufferedInputFile(image_bytes, filename="hd_image.png")

        # Model selection buttons
        buttons = []
        row = []
        for k in IMAGE_MODELS:
            if k == model:
                continue
            row.append(InlineKeyboardButton(
                text=f"🎨 {k}",
                callback_data=f"hd:{k}:{actual_prompt[:40]}",
            ))
            if len(row) == 2:
                buttons.append(row)
                row = []
        ...
        logger.error("HD image failed: %s", exc)
        await safe_edit_text(status, f"⚠️ خطا:\n`{str(exc)[:200]}`")


@router.callback_query(F.data.startswith("hd:"))
async def cb_hd_model(callback: CallbackQuery) -> None:
    from arki_project.utils.image_gen import generate_image_hd
    await callback.answer(f"🖼 ساخت...")
    parts = callback.data.split(":", 2)
    model = parts[1]
    prompt = parts[2] if len(parts) > 2 else "beautiful image"
    try:
        img = await generate_image_hd(prompt, model=model)
        photo = BufferedInputFile(img, filename=f"hd_{model}.png")
        await callback.message.answer_photo(
            photo=photo,
            caption=f"🖼 *HD — {prompt[:60]}*\n_Model: {model}_",
            parse_mode="Markdown",
        )
    except HandlerError as exc:
...
# ═══════════════════════════════════════
# /style — Multi-Style Generation (v30.1)
# ═══════════════════════════════════════

@router.message(Command("style"))
async def cmd_multi_style(message: Message) -> None:
    """Generate image in 5 different art styles simultaneously."""
    from arki_project.utils.image_gen import generate_image_multi_style

    prompt = extract_args(message.text or "", "/style")
    if not prompt:
        await safe_reply(message, "🎨 *تولید چند‌سبکی:*\n\n"
            "Usage: `/style [توضیح]`\n\n"
            "یک پرامپت → ۵ سبک مختلف:\n"
            "  🖼 Realism | 🎌 Anime | 🧊 3D\n"
            "  🎨 Standard | ✨ CablyAI\n\n"
            "*مثال:*\n"
            "`/style a beautiful candle on a table`\n"
            "`/style غروب خورشید روی کوه`\n\n"
            "_همه ۵ سبک همزمان ساخته می‌شن!_")
        return

    await message.bot.send_chat_action(
        chat_id=message.chat.id, action=ChatAction.UPLOAD_PHOTO,
    )
    status = await message.answer("🎨 دارم ۵ سبک مختلف می‌سازم...")

    try:
        results = await generate_image_multi_style(prompt)
        ...
        for img_bytes, style_name in results:
            label = style_labels.get(style_name, f"🎨 {style_name}")
            photo = BufferedInputFile(img_bytes, filename=f"style_{style_name}.png")
            await message.answer_photo(
                photo=photo,
                caption=f"{label}: *{prompt[:80]}*",
                parse_mode="Markdown",
            )
        ...
        logger.error("Multi-style failed: %s", exc)
        await safe_edit_text(status, f"⚠️ خطا:\n`{str(exc)[:200]}`")
